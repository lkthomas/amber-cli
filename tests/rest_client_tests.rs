...
async fn test_valid_json_parsing_for_site_details() {
    let mock_server = MockServer::start().await;
    let template =
        ResponseTemplate::new(200).set_body_raw(mock_amber_site_data::body(), "application/json");
    let mut user_site_details = RestClient::new_client(mock_server.uri(), "token".to_string());
    ...
    assert_eq!(
        test_user_site_data,
        &mock_site_details_struct::json_response()
    );
}
